  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ongoing = await azle.hostedOngoingLotteries(principal);
        if ("Ok" in ongoing) {
          setOngoing(ongoing.Ok);
        }
        const completed = await azle.hostedCompletedLotteries(principal);
        if ("Ok" in completed) {
  ...
  return (
    <>
      <Flex direction={"column"} gap={"2rem"}>
        ...
        <Flex direction={"column"} gap={"1rem"}>
          <Flex>
            <h1 className="text-3xl font-bold">Completed Hosted Lotteries</h1>
            <Spacer />
            <Link
              href="/lotteries/hosted/completed"
              className="underline font-semibold"
            >
              See all
            </Link>
